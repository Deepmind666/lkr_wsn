...
def one_run(args):
    (seed, cfg_tuple, xs, ys, minx, miny, loader, name, opts) = args
    random.seed(seed); np.random.seed(seed)
    cfg = NetworkConfig(*cfg_tuple)
    ...
    return (name, res.get('total_energy_consumed', 0.0), res.get('packet_delivery_ratio_end2end', 0.0))

if __name__ == '__main__':
    repeats = int(sys.argv[1]) if len(sys.argv) > 1 else 100
    ...
    n = len(locs)
    cfg_tuple = (n, width, height, 2.0, 1024)  # NetworkConfig signature order

    tasks = []
    for name, opts in VARIANTS.items():
        for r in range(repeats):
            seed = 1000 + r
            tasks.append((seed, cfg_tuple, xs, ys, minx, miny, loader, name, opts))

    energies = {k: [] for k in VARIANTS}
    pdrs = {k: [] for k in VARIANTS}

    with cf.ProcessPoolExecutor(max_workers=workers) as ex:
        for name, e, p in ex.map(one_run, tasks, chunksize=1):
            energies[name].append(e)
            pdrs[name].append(p)

    summary = {}
    for name in VARIANTS:
        mean_e, ci_e = _def(energies[name])
        mean_p, ci_p = _def(pdrs[name])
        summary[name] = {
        ...
        print(f"[ABLT|PAR] {name}: energy={mean_e:.3f}±{ci_e:.3f}, pdr={mean_p:.3f}±{ci_p:.3f}")

    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'intel_ablation_parallel.json')
