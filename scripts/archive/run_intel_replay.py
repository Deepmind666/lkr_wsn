...
BASE_SEED = int(os.environ.get('AERIS_SEED', '133742'))
if __name__ == '__main__':
    ...
    def run_profile(profile: str, seed_offset: int = 0):
        ...
        result['config']['runtime']['seed'] = seed
        return result

    logger = ExperimentLogger()
    res_energy = run_profile('energy', seed_offset=0)
    logger.log('intel_replay', {'profile':'energy', 'seed': BASE_SEED}, res_energy)
    res_robust = run_profile('robust', seed_offset=1)
    logger.log('intel_replay', {'profile':'robust', 'seed': BASE_SEED + 1}, res_robust)
    ...
    out = {
        ...
        'runtime': {
            'energy': res_energy.get('config', {}).get('runtime', {}),
            'robust': res_robust.get('config', {}).get('runtime', {})
        },
