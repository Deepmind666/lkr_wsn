...
import numpy as np
BASE_SEED = int(os.environ.get('AERIS_SEED', '41001'))
...
def one_run(args):
    (seed, cfg_kwargs, xs, ys, minx, miny, loader, name, opts) = args
...
if __name__ == '__main__':
    ...
    for idx, (name, opts) in enumerate(VARIANTS.items()):
        for r in range(repeats):
            ...
            tasks.append((seed, cfg_kwargs, xs, ys, minx, miny, loader, name, opts))
    ...
    for name in VARIANTS:
        ...
        summary[name] = {
            'energy': {'mean': mean_e, 'ci95': ci_e, 'values': energies[name]},
            'pdr_end2end': {'mean': mean_p, 'ci95': ci_p, 'values': pdrs[name]},
