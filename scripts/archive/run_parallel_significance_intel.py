...
BASE_SEED = int(os.environ.get('AERIS_SEED', '43001'))
...
def one_run(args):
    (profile, xs, ys, cfg_dict, regimes, t_med, seed) = args
    import random
    random.seed(seed)
    np.random.seed(seed)
...
if __name__ == '__main__':
    ...
    # Conservative env mapping regimes from dataset percentiles
    ...
    # Prepare tasks
    # allow CLI override: python run_parallel_significance_intel.py [repeats] [workers]
    repeats = int(sys.argv[1]) if len(sys.argv) > 1 else int(os.environ.get('INTEL_REPEATS', '20'))
    # workers override via argv[2] or env INTEL_WORKERS
    if len(sys.argv) > 2:
        try:
            user_workers = int(sys.argv[2])
        except Exception:
            user_workers = None
    else:
        user_workers = None
    env_workers = None
    try:
        if 'INTEL_WORKERS' in os.environ:
            env_workers = int(os.environ['INTEL_WORKERS'])
    except Exception:
        env_workers = None
    cpu_default = max(1, (os.cpu_count() or 4) - 1)
    max_workers = user_workers or env_workers or cpu_default
    ...
    tasks = []
    for ssd in seeds:
        tasks.append(('energy', xs, ys, cfg_dict, regimes, t_med, ssd))
        tasks.append(('robust', xs, ys, cfg_dict, regimes, t_med, ssd))

    # Run in parallel
    base, robust = [], []
    print(f"[Intel Parallel Significance] repeats={repeats}, workers={max_workers}")
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                print('Task failed for', t[0], 'seed', t[-1], 'err=', e)
    ...
    report['meta'] = {'base_seed': BASE_SEED, 'repeats': repeats, 'workers': max_workers, 'seed_list': seeds}
    out = os.path.join(os.path.dirname(__file__), '..', 'results', 'significance_compare_intel_parallel.json')
    os.makedirs(os.path.dirname(out), exist_ok=True)
    json.dump(report, open(out,'w',encoding='utf-8'), ensure_ascii=False, indent=2)

    print('Saved', out)
    print('Elapsed: %.1fs, repeats=%d, workers=%d' % (time.time()-t0, repeats, max_workers))
